...
(function () {
  ...
  function addChatMessage(text, isUser = false) {
    ...
    msg.innerHTML = `
      ...
      <div class="chat-bubble">${isUser ? escapeHtml(text) : formatChatText(normalizeChatReply(text))}</div>
    `;
  ...
  function renderTeam() {
    ...
    teamGrid.querySelectorAll('.team-card-btn').forEach(btn => {
      btn.addEventListener('click', function () {
        ...
        if (member) openTeamPopup(member);
      });
    });
  }
  ...
  function openTeamPopup(member) {
    ...
    if (!overlay) return;
    $('#teamPopupImg').src = member.image;
    $('#teamPopupName').textContent = member.name;
    $('#teamPopupRole').textContent = member.role;
    $('#teamPopupBio').textContent = member.bio;
  ...
  // Close popup on X click, overlay click, or Escape key
  const _tpClose = $('#teamPopupClose');
  if (_tpClose) _tpClose.addEventListener('click', closeTeamPopup);

  const _tpOverlay = $('#teamPopup');
  if (_tpOverlay) {
    _tpOverlay.addEventListener('click', function (e) {
